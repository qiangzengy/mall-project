...
public class SeckillServiceImpl implements SeckillService {
    ...
    public void seckillGoodsUp() {
        ...
        l.add("234");
        l.add("456");
        Long abcd = redisTemplate.opsForList().leftPushAll("abcd", l);
        System.out.println("******************"+abcd);
        System.out.println(redisTemplate.opsForList().range("abcd",0,-1));
    ...
    private void saveSessInfos(List<SeckillSessionVo> list) {
        list.stream().forEach(session -> {
            ...
            if (!aBoolean) {
                ...
                // 从左边放
                redisTemplate.opsForList().leftPushAll(SESSION_PREFIX + key, collect);
    ...
    private void saveSessionSkuInfos(List<SeckillSessionVo> list) {
        list.stream().forEach(session -> {
            // hash
            BoundHashOperations<String, String, String> operations = redisTemplate.boundHashOps(SESSION_ITEM);
            // 商品信息
            ...
            for (SeckillSkuRelationVo entity : seckillSkuRelationEntities) {
                ...
                Boolean aBoolean = redisTemplate.hasKey(entity.getPromotionSessionId() + "_" + entity.getSkuId());
                if (!aBoolean) {
